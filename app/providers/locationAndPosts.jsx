...
export const LocationAndPostsProvider = ({ children }) => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            lastLocationRef.current = localStorage.getItem('lastLocation') ? JSON.parse(localStorage.getItem('lastLocation')) : null;
    ...
    const handleSuccess = useCallback((position) => {
        if (position && position.coords) {
            ...
            if (!lastLocationRef.current || calculateDistance(lastLocationRef.current, newLocation) >= 50) {
    ...
    useEffect(() => {
        const watchId = navigator.geolocation && navigator.geolocation.watchPosition(handleSuccess, handleError);
        return () => {
    ...
    return (
        <LocationAndPostsContext.Provider value={contextValue}>
            {children}
